...
public interface UserScoreRepository extends CrudRepository<UserScore, Long> {
    ...
    @Query("SELECT u FROM UserScore u WHERE u.resultQuiz.name = :quizName and u.student.id = :id")
    UserScore getScoreForUserByQuiz(@Param("quizName") String quizName, @Param("id") Long id);

    @Query("SELECT u.student FROM UserScore u WHERE u.teacherName = :teacherName and u.resultQuiz.name = :quizName")
    ...
    @Query("SELECT u FROM UserScore u WHERE u.resultQuiz.name = :quizName and u.student.id = :studentId and u.teacherName = :teacherName")
    ...
    @Query("SELECT u FROM UserScore u WHERE u.resultQuiz.name = :quizName and u.student.id = :studentId")
